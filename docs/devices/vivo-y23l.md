...
| USB 串口控制台 | 🧪 | 与红米 2 相同（configfs gadget + ncm-serial）；dtb 照抄老项目实测可用的 device 版：`dr_mode=otg`，extcon 双槽都给 ID GPIO，`pm8916_usbin` 保持 disabled；另需 `CONFIG_CHARGER_SMB347=m`（vbus-supply 在 smb358 节点里，缺驱动则 ci_hdrc 永不 probe） |
| 基带 | ❌ | 未做 |
